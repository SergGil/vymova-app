...
vi.mock('../../js/features/duel.ts', async (importOriginal) => {
  ...
  return { ...orig, _getResultData: getResultData, _onResultRematch: onResultRematch, _onResultNewDuel: onResultNewDuel, _onResultReaction: onResultReaction };
});
...
describe('duel-result.tsx DuelResult', () => {
  ...
  it('renders a round result with series score and "next round" button', () => {
    getResultData.mockReturnValue({ kind: 'round', outcome: 'win', round: 1, myWins: 1, oppWins: 0, myName: 'Alice', oppName: 'Bob' });
    const { container, root } = mount();
    roots.push(root);
  ...
  it('renders a final win result with scores and avatars', () => {
    getResultData.mockReturnValue({
      kind: 'final', outcome: 'win', modeIcon: '🧠', modeLabel: 'Тест', catLabel: '',
      myAvatar: '🧑', myScore: 8, oppAvatar: '🤖', oppScore: 5, oppName: 'Bot',
      roomSize: 10, historyText: '',
    });
    const { container, root } = mount();
    roots.push(root);
  ...
  it('renders a final loss result with the opponent name', () => {
    getResultData.mockReturnValue({
      kind: 'final', outcome: 'loss', modeIcon: '🧠', modeLabel: 'Тест', catLabel: '',
      myAvatar: '🧑', myScore: 3, oppAvatar: '🤖', oppScore: 8, oppName: 'Bot',
      roomSize: 10, historyText: '',
    });
    const { container, root } = mount();
    roots.push(root);
  ...
  it('rematch/new-duel buttons and emoji reactions call their handlers', () => {
    getResultData.mockReturnValue({
      kind: 'final', outcome: 'tie', modeIcon: '🧠', modeLabel: 'Тест', catLabel: '',
      myAvatar: '🧑', myScore: 5, oppAvatar: '🤖', oppScore: 5, oppName: 'Bot',
      roomSize: 10, historyText: '',
    });
    const { container, root } = mount();
    roots.push(root);

    const rematchBtn = Array.from(container.querySelectorAll('button')).find(b => b.textContent?.includes('Реванш')) as HTMLButtonElement;
    act(() => { rematchBtn.click(); });
    expect(onResultRematch).toHaveBeenCalled();

    const newDuelBtn = Array.from(container.querySelectorAll('button')).find(b => b.textContent?.includes('Нова дуель')) as HTMLButtonElement;
    act(() => { newDuelBtn.click(); });
