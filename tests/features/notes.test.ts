...
describe('notes.ts', () => {
  ...
  it('returns empty string and false for a word with no note', async () => {
    const { getNoteForWord, hasNote } = await import('../../js/features/notes/notes.ts');
  ...
  it('saves a note, trims it, and persists to localStorage', async () => {
    const { getNoteForWord, hasNote, setNoteForWord } = await import('../../js/features/notes/notes.ts');
