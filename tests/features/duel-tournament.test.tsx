...
const { getTournamentData, getDuelScreen, onTournStart, onTournCancel, onTournPlay, onTournRejoin } = vi.hoisted(() => ({
...
function mount(): { container: HTMLElement; root: Root } {
  ...
  act(() => { root.render(<DuelTournament />); });
...
describe('duel-tournament.tsx DuelTournament', () => {
  ...
  it('renders the waiting room with slots and a start button', () => {
    ...
    getTournamentData.mockReturnValue({
      phase: 'waiting', code: 'ABC-123', modeLabel: '🧠 Тест · 4 гравці',
      slots: [
        { filled: true, avatar: '🧑', name: 'Alice', label: '' },
        { filled: false, avatar: '', name: '', label: 'Слот 2' },
      ],
      joined: 1, size: 4,
      showStartBtn: true, startBtnLabel: '⚔️ Почати турнір (1/4)',
    ...
    const startBtn = Array.from(container.querySelectorAll('button')).find(b => b.textContent?.includes('Почати турнір')) as HTMLButtonElement;
    act(() => { startBtn.click(); });
    expect(onTournStart).toHaveBeenCalled();

    const cancelBtn = Array.from(container.querySelectorAll('button')).find(b => b.textContent?.includes('Скасувати')) as HTMLButtonElement;
    act(() => { cancelBtn.click(); });
  ...
  it('renders the bracket with rounds and a champion', () => {
    getDuelScreen.mockReturnValue('tournament');
    getTournamentData.mockReturnValue({
      phase: 'bracket', code: 'ABC-123', modeLabel: '', slots: [], joined: 4, size: 4,
      showStartBtn: false, startBtnLabel: '',
      finished: true, champion: 'Alice', statusLabel: 'Завершено', statusColor: '#f39c12',
      rounds: [{
        name: 'Фінал',
        matches: [{ p1: { avatar: '🧑', name: 'Alice', won: true }, p2: { avatar: '🦊', name: 'Bob', won: false }, scoreText: '8:5', active: false }],
      }],
    ...
    const leaveBtn = Array.from(container.querySelectorAll('button')).find(b => b.textContent?.includes('Вийти')) as HTMLButtonElement;
    act(() => { leaveBtn.click(); });
    expect(onTournCancel).toHaveBeenCalled();
  });
  ...
  it('renders a play button for the matchArea "play" state', () => {
    getDuelScreen.mockReturnValue('tournament');
    getTournamentData.mockReturnValue({
      phase: 'bracket', code: '', modeLabel: '', slots: [], joined: 4, size: 4,
      finished: false, champion: '', statusLabel: '', statusColor: '',
      rounds: [], matchArea: { kind: 'play' }, showStartBtn: false, startBtnLabel: '',
    });
    const { container, root } = mount();
    roots.push(root);
    const playBtn = Array.from(container.querySelectorAll('button')).find(b => b.textContent?.includes('Грати зараз')) as HTMLButtonElement;
    act(() => { playBtn.click(); });
  ...
  it('renders a rejoin button for the matchArea "rejoin" state', () => {
    ...
    getTournamentData.mockReturnValue({
      phase: 'bracket', code: '', modeLabel: '', slots: [], joined: 4, size: 4,
      finished: false, champion: '', statusLabel: '', statusColor: '',
      rounds: [], matchArea: { kind: 'rejoin' }, showStartBtn: false, startBtnLabel: '',
    });
    const { container, root } = mount();
    roots.push(root);
    const rejoinBtn = Array.from(container.querySelectorAll('button')).find(b => b.textContent?.includes('Продовжити матч')) as HTMLButtonElement;
    act(() => { rejoinBtn.click(); });
  ...
  it('renders the waiting-for-match message for the matchArea "waiting" state', () => {
    ...
    getTournamentData.mockReturnValue({
      phase: 'bracket', code: '', modeLabel: '', slots: [], joined: 4, size: 4,
      finished: false, champion: '', statusLabel: '', statusColor: '',
      rounds: [], matchArea: { kind: 'waiting', oppName: 'Bob' }, showStartBtn: false, startBtnLabel: '',
    });
    const { container, root } = mount();
    roots.push(root);
