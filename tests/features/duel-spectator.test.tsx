...
vi.mock('../../js/features/duel.ts', async (importOriginal) => {
  ...
  return { ...orig, _getSpecRoom: getSpecRoom, _getDuelScreen: getDuelScreen, _leaveSpectator: leaveSpectator };
});
...
function mount(): { container: HTMLElement; root: Root } {
  ...
  act(() => { root.render(<DuelSpectatorView />); });
  return { container, root };
}

const baseRoom = {
  seed: 1, mode: 'quiz', category: '', difficulty: 'mixed',
  bestOf: 1, maxHints: 3, powerupsEnabled: false,
  p1: { name: 'Alice', avatar: '🧑', score: 5, idx: 5, done: false, hintsLeft: 3, powerups: { double: 1, skip: 1, freeze: 1 } },
  p2: { name: 'Bob', avatar: '🦊', score: 3, idx: 3, done: false, hintsLeft: 3, powerups: { double: 1, skip: 1, freeze: 1 } },
  started: true, finished: false, createdAt: 0,
