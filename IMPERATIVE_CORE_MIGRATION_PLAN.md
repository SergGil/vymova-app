...
- **[x] Batch 2: `checkAchievements`, `speak`, `_speakWithLang`** —
  ...
  536/536, tsc чистий, 8/8 e2e.
- **[ ] Залишок** (~14 `window.*`, переважно читачі — `i18n.ts`/
  `sidebar.ts`): `_renderVoices`, `openLearningPath`,
  `renderLearningPath`, `_refreshRangeOptions`, `invalidateCatCache`,
  `_renderWeakWords`, `invalidateReadingIndex`, `openGrammarContent`,
  `_refreshIdiomsUI`, `_refreshPrefetchUI`, `_refreshPixabayStatus`,
  `_refreshCloudSyncUI`, `_refreshNotifUI`, `renderStats`,
  `_refreshStatsExtras` — типово цикл-чутливі (i18n.ts/sidebar.ts
  імпортуються майже всюди), потребують динамічних імпортів за зразком
  Фази 7.5. Окрема майбутня робота.
