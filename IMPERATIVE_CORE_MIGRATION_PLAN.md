...
- [x] Фаза 9/5 — DuelOverlay рерайт: Spectator/Tournament conditional render
- [x] Фаза 9/6 — DuelOverlay рерайт: Lobby → `duel-lobby.tsx`
- [x] Фаза 9/7 — DuelOverlay parent + cleanup
...
- **[x] 9/5 — Spectator/Tournament conditional render**: прибрано
  `.style.display`-обгортки `#duel-spectate`/`#duel-tournament` —
  `flashcard_trainer_starwars.html` тепер містить лише `*-mount` div'и
  напряму. `DuelSpectatorView`/`DuelTournament` самі повертають `null`,
  якщо `_getDuelScreen() !== 'spectate'/'tournament'` (новий експорт
  `_getDuelScreen` використано напряму). У `duel.ts` видалено
  `_hideTournament()` (3 викликів) та ручні toggle'и `el.style.display`
  у `_startSpectatorView`/`_leaveSpectator`/`_showTournament`; smart
  close-кнопка (`#duel-page-close`) тепер визначає
  `tournVisible`/`spectVisible` через `state.duelScreen` замість
  `.style.display`.
  536/536, tsc чистий, 8/8 e2e, ручний smoke-тест (Playwright) підтвердив
  коректне перемикання екранів без помилок.

- **[x] 9/6 — Lobby → `duel-lobby.tsx`**: новий `state.duelLobbyUI`
  (`AppState.duelLobbyUI: DuelLobbyUIState`, дефолт у `src/state.ts`) —
  msg-бокс (текст або "challenge accepted"-картка з
  avatar/name/modeIcon/modeLabel), waiting-банер
  (roomCode/modeLabel), `joinRowVisible`, disabled-стани кнопок
  create/join/async/tourn4/tourn8 (+`errorLabel` для турнірних кнопок).
  Геттер `_getLobbyUIData()`. Конвертовано на `state.duelLobbyUI` +
  `notifyStateChange()`: `_showLobby`, `createRoom`, `joinRoom` (тепер
  `joinRoom(rawCode: string)` — код кімнати став контрольованим
  React-інпутом), `_cancelRoom`, `joinAsSpectator`,
  `createAsyncChallenge`, `joinAsyncChallenge`, `_doRematch`,
  `createTournament(size)`, `joinTournament` — усі експортовано.
  Видалено `elMsg()` та 9 ручних `addEventListener`
  (create/join/cancel/spectate/async/async-join/tourn-create×2/
  tourn-join) + keydown-форматер `#duel-join-input` (логіка форматування
  коду — strip/uppercase/dash/maxlen — перенесена в `onChange`
  контрольованого інпута в `duel-lobby.tsx`).
  Новий `js/features/duel-lobby.tsx` — обгортка над 6 React-пікерами
  (`DuelLeaderboard`/`DuelRating`/`DuelHistory`/`DuelModePicker`/
  `DuelCategoryPicker`/`DuelOptionsRow`/`DuelResume`) + усі
  create/join/cancel/spectate/async/tournament-кнопки як JSX з
  `onClick`/`onChange`/`onKeyDown`. Кореневий `id="duel-lobby"` та всі
  e2e-залежні внутрішні `id` (`#duel-mode-picker`/`#duel-cat-picker`/
  `#duel-options-row`/`#duel-resume-mount`/`#duel-msg`/`#duel-create-btn`/
  `#duel-join-btn`/`#duel-cancel-btn`/`#duel-waiting`/`#duel-room-code`/
  `#duel-join-input`) збережені на тих самих елементах.
  `flashcard_trainer_starwars.html`: блок `#duel-lobby` (рядки 602-674)
  замінено на `<div id="duel-lobby-mount"></div>`; в `app-root.tsx`
  видалено 7 окремих `<Portal>` (тепер діти `DuelLobby`), додано
  `<Portal id="duel-lobby-mount"><DuelLobby/></Portal>`.
  Нові permanent-тести: `tests/features/duel-lobby-ui-state.test.ts`
  (`_getLobbyUIData()` дефолти + `_cancelRoom()` resets), новий e2e-кейс
  у `tests-e2e/duel-room.spec.ts` ("cancelling a created room returns to
  the join/create lobby state").
  538/538, tsc чистий, 9/9 e2e.

- **[x] 9/7 — `DuelOverlay` parent + cleanup**: новий
  `js/features/duel-overlay.tsx` умовно рендерить екран дуелі за
  `_getDuelScreen()` (`DuelLobby` лише на `'lobby'`, countdown-/game-/
  chat-/result-обгортки лише на відповідному екрані;
  `DuelSpectatorView`/`DuelTournament` самі повертають `null` для інших
  екранів). У `flashcard_trainer_starwars.html` весь блок `#duel-overlay`
  після `.page-header` (14 окремих `*-mount` div'ів + `#duel-countdown`/
  `#duel-game`/`#duel-chat-panel`/`#duel-result` обгортки) замінено на
  один `<div id="duel-overlay-mount"></div>`; в `app-root.tsx` 14 окремих
  `<Portal>` замінено на один `<Portal id="duel-overlay-mount">
  <DuelOverlay/></Portal>`.
  У `duel.ts` видалено `elLobby`/`elCountdown`/`elGame`/`elResult`/
  `elChatPanel` та всі залишкові `.style.display` toggle'и в
  `_showLobby`/`_showCountdown`/`_showGame`/`_showResult`/
  `_showTournament`/`_startSpectatorView` (лишилось лише
  `state.duelScreen=...; notifyStateChange()`); smart close-кнопка
  (`#duel-page-close`) та document-level quiz keydown тепер визначають
  `gameVisible`/`countdownVisible` через `state.duelScreen` замість
  `.style.display`. Tooltip (`_showInfoTooltip`, рядки ~359-410) лишено
  без змін (низький пріоритет, поза скоупом).
  538/538, tsc чистий, 9/9 e2e, ручний smoke-тест (Playwright, скріншоти
  lobby/waiting) підтвердив рендер без помилок.

DuelOverlay рерайт (Фаза 9) повністю завершено (9/1-9/7).
