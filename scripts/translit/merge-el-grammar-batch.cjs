// One-off batch merge/verify script for expanding data/grammar-data/grammar_el.ts.
// Usage: node scripts/translit/merge-el-grammar-batch.cjs <batch.json>
...
if (!batchPath) {
  console.error('Usage: node merge-el-grammar-batch.cjs <batch.json>');
  process.exit(1);
}

const filePath = path.join(__dirname, '..', '..', 'data', 'grammar-data', 'grammar_el.ts');
...
console.log(`Added ${batch.rules.length} rules to category "${batch.categoryId}". Total rules now: ${totalRules} across ${data.length} categories.`);
