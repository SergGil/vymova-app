// One-off batch merge/verify script for the Dutch (nl) word-list expansion.
...
if (!batchPath) {
  console.error('Usage: node merge-nl-batch.cjs <batch.json>');
  process.exit(1);
}
...
const W_NL = loadArr(nlPath, 'W_NL');
...
for (const item of batch) {
  ...
  if (Object.prototype.hasOwnProperty.call(W_NL, word)) {
  ...
  W_NL[word] = [translation, example, ipa];
  added++;
}
...
for (const w of baseWords) {
  if (Object.prototype.hasOwnProperty.call(W_NL, w)) {
    ordered[w] = W_NL[w];
  }
}

const header = `// Vymova — Dutch translations (subset)
...
fs.writeFileSync(nlPath, out, 'utf8');
