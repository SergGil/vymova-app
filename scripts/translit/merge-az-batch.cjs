// One-off batch merge/verify script for the Azerbaijani (az) word-list expansion.
// Loads a batch JSON of [english, azerbaijani_translation, azerbaijani_example]
// tuples, validates against the base word list, merges into words_az.js in
...
if (!batchPath) {
  console.error('Usage: node merge-az-batch.cjs <batch.json>');
  process.exit(1);
}

const basePath = path.join(__dirname, '../../data/words-data/words.js');
const azPath = path.join(__dirname, '../../data/words-data/words_az.js');
...
const W_AZ = loadArr(azPath, 'W_AZ');
...
for (const item of batch) {
  ...
  if (Object.prototype.hasOwnProperty.call(W_AZ, word)) {
...
for (const w of baseWords) {
  if (Object.prototype.hasOwnProperty.call(W_AZ, w)) {
    ordered[w] = W_AZ[w];
  }
}

const header = `// Vymova — Azerbaijani translations (subset)
...
fs.writeFileSync(azPath, out, 'utf8');
