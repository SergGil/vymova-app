      ...
      {d.showOptions && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
          {d.options.map((opt) => (
            ...
            >
              <span className="opt-num inline-flex shrink-0 items-center justify-center rounded-[5px] bg-[var(--border)] mr-1.5 align-middle text-[.68rem] font-bold text-[var(--text2)] group-[.correct]:hidden group-[.wrong]:hidden group-[.reveal]:hidden size-[18px]">{opt.num}</span> {opt.text}
            </button>
          ))}
        </div>
      )}
