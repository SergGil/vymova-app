  ...
  return (
    <>
      ...
      >
        ...
        {d.qTertiary && (
          <div
            style={{ fontSize: '.78rem', color: 'var(--text3)', marginTop: 4, textAlign: 'center' }}
          >
            {d.qTertiary}
          </div>
        )}
      ...
      {d.showOptions && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
          {d.options.map((opt) => (
            ...
            >
              <span className="opt-num inline-flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-[5px] bg-[var(--border)] mr-1.5 align-middle text-[.68rem] font-bold text-[var(--text2)] group-[.correct]:hidden group-[.wrong]:hidden group-[.reveal]:hidden">{opt.num}</span> {opt.text}
            </button>
          ))}
        </div>
      )}
      ...
      {d.showInputRow && (
        <div style={{ marginBottom: 10 }}>
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && !d.answered) _submitWrite();
            }}
            style={{
              width: '100%',
              padding: '11px 14px',
              border: `2px solid ${d.inputBorderColor || 'var(--border)'}`,
              borderRadius: 12,
              fontSize: '1rem',
              fontFamily: 'inherit',
              background: 'var(--bg)',
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              ref={nextRef}
              style={{
                display: d.showNextBtn ? 'inline-block' : 'none',
                padding: '10px 18px',
                borderRadius: 10,
                border: 'none',
                background: 'var(--accent)',
