...
export function speakForCode(code: Code, text: string, fallbackEnText: string, btn: HTMLElement | null): void {
  if (isTargetLang(code)) {
    const cfg = langConfig(code);
    if (text && VOICE_GETTERS[code]()) { _speakWithLang(text, cfg.voiceLocale, btn); return; }
    speak(fallbackEnText, btn);
    return;
  }
  if (code === 'ua') {
    if (text && getSelectedUkVoice()) { _speakWithLang(text, 'uk-UA', btn); return; }
    speak(fallbackEnText, btn);
    return;
  }
