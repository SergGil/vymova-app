      ...
      {POWERUPS.map(p => {
        ...
        return (
          <button
            ...
            title={titleText}
            style={{
              padding: '5px 8px', borderRadius: 9, border: `1.5px solid ${borderColor}`, background: bgColor,
              cursor: canUse ? 'pointer' : 'default', fontSize: '.78rem', color: textColor, opacity,
              // pointer-events замість disabled: на iOS Safari disabled-кнопка
              // все ще ковтає тапи, що може блокувати сусідню кнопку в рядку.
              pointerEvents: canUse ? 'auto' : 'none', transition: 'opacity .2s', fontFamily: 'inherit',
            }}
            onClick={() => { if (canUse) _onPowerupClick(p.id as PowerupType); }}
          >
