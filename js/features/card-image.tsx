...
function renderCardImage(word: string, illusEl: HTMLElement, isCurrentWord: (w: string) => boolean): void {
  try {
    if (_imgCache.hasOwnProperty(word) && _imgCache[word]) {
      ...
      const _clearAndRefetch = function() {
        ...
        if (typeof _idb !== 'undefined' && _idb) {
          try { _idb.transaction('imgs', 'readwrite').objectStore('imgs').delete(word); } catch (e2) {}
        }
        const fb = getIllus(word);
        if (fb) { illusEl.innerHTML = fb; illusEl.style.display = ''; }
        ...
        loadWikiImage(word, function(wd, newUrl) {
          ...
          if (newUrl) {
            ...
            _ni.onload  = function() { if (_ni.naturalWidth < 10) illusEl.style.display = 'none'; };
            _ni.onerror = function() { illusEl.style.display = 'none'; };
            _ni.src = newUrl;
            illusEl.innerHTML = ''; illusEl.appendChild(_ni); illusEl.style.display = '';
          }
        });
      };
