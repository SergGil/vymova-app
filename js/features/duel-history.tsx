...
export function DuelHistory() {
  ...
  const h = _getHistory();
  if (!h.length) {
    return <div style={{ color: 'var(--text3)', fontSize: '.8rem', textAlign: 'center', padding: 8 }}>{t('duel.noHistory')}</div>;
  }
  ...
  return <>
    ...
    {pages > 1 && (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10, paddingTop: 8, fontSize: '.78rem', color: 'var(--text2)' }}>
        <button type="button" disabled={curPage === 0} onClick={() => setPage(curPage - 1)}
          style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 4, color: 'inherit', padding: '2px 8px', cursor: 'pointer' }}>‹</button>
        <span>{curPage + 1} / {pages}</span>
        <button type="button" disabled={curPage >= pages - 1} onClick={() => setPage(curPage + 1)}
          style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 4, color: 'inherit', padding: '2px 8px', cursor: 'pointer' }}>›</button>
      </div>
    )}
  </>;
