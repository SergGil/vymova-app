...
const againBtnStyle = { padding: '12px 24px', borderRadius: 12, border: 'none', background: 'var(--accent)', color: '#fff', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', fontSize: '.88rem' };
...
export function DuelResult(): ReactElement | null {
  ...
  if (d.kind === 'round') {
    const seriesLabel = d.outcome === 'win' ? t('duel.series.win') : d.outcome === 'tie' ? t('duel.series.tie') : t('duel.series.loss');
    return (
      <>
        ...
        <div style={btnRowStyle}>
          <button style={rematchBtnStyle} onClick={_onResultRematch}>{t('duel.nextRound')}</button>
        </div>
      </>
    );
  }
  ...
  const resultLabel = d.outcome === 'win' ? t('duel.result.won') : d.outcome === 'tie' ? t('duel.result.tie') : t('duel.result.lost', { name: d.oppName });

  return (
    <>
      <div style={{ fontSize: '.72rem', color: 'var(--text3)', marginBottom: 6 }}>{d.modeIcon} {d.modeLabel}{d.catLabel}</div>
      <div style={{ fontSize: '3rem', marginBottom: 8 }}>{OUTCOME_EMOJI[d.outcome]}</div>
      <div style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--text)', marginBottom: 6 }}>{resultLabel}</div>
      <div style={{ display: 'flex', gap: 20, justifyContent: 'center', margin: '14px 0' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: '2rem' }}>{d.myAvatar}</div>
          <div style={{ fontWeight: 700, fontSize: '1.2rem', color: myColor }}>{d.myScore}/{d.roomSize}</div>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontWeight: 700, fontSize: '1.2rem', color: oppColor }}>{d.oppScore}/{d.roomSize}</div>
          <div style={{ fontSize: '.72rem', color: 'var(--text3)' }}>{d.oppName}</div>
        </div>
      </div>

      <div style={{ fontSize: '.75rem', color: 'var(--text3)', margin: '10px 0 0' }}>{d.historyText}</div>

      <div style={btnRowStyle}>
        <button style={rematchBtnStyle} onClick={_onResultRematch}>{t('duel.rematch')}</button>
        <button style={againBtnStyle} onClick={_onResultNewDuel}>{t('duel.newDuel')}</button>
      </div>

      <div style={{ marginTop: 14 }}>
        <div style={{ fontSize: '.7rem', color: 'var(--text3)', marginBottom: 6 }}>{t('duel.sendReaction')}</div>
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
          {REACTIONS.map(e => (
            <button key={e} style={{ fontSize: '1.5rem', background: 'none', border: 'none', cursor: 'pointer', padding: 4 }} onClick={() => _onResultReaction(e)}>{e}</button>
          ))}
        </div>
      </div>
    </>
