...
function computeHeatmap(): HeatDay[][] {
  ...
  return weeks.map(wk => wk.map(day => ({
    ...day,
    lvl: day.n === 0 ? 0 : day.n < maxN * 0.25 ? 1 : day.n < maxN * 0.5 ? 2 : day.n < maxN * 0.75 ? 3 : 4,
  })));
}
...
function computeMonthCal(year: number, month: number): { cells: CalCell[]; summary: string } {
  ...
  const summary = monthTotal > 0 ? `${t('stats.totalForMonth')}: ${monthTotal} ${wordsLabel(monthTotal)}` : t('stats.noWordsThisMonth');
...
// ── Mode accuracy ─────────────────────────────────────────────
type ModeRow = { key: string; icon: string; label: string; pct: number | null; totText: string; sessions: number; barColor: string };
...
// ── CEFR stats ─────────────────────────────────────────────────
type CefrRow = { level: string; desc: string; color: string; known: number; total: number; pct: number };
  ...
  return (
    <div className="stats-panel">
      <div className="stats-header">
        <div className="stats-title" data-i18n="stats.title">{t('stats.title')}</div>
        <button className="stats-close" id="stats-close" onClick={closeStats}>✕</button>
      </div>

      <div className="stats-section">
        <div className="stats-section-title" data-i18n="stats.overallProgress">{t('stats.overallProgress')}</div>
        <div className="stats-summary">
          <div className="stat-card"><div className="sv" id="st-known">{knownCount}</div><div className="sl" data-i18n="stats.wordsLearned">{t('stats.wordsLearned')}</div></div>
          <div className="stat-card"><div className="sv" id="st-pct">{pctKnown}%</div><div className="sl" data-i18n="stats.ofAllWords">{t('stats.ofAllWords')}</div></div>
          <div className="stat-card"><div className="sv" id="st-streak">{gd.streak || 0}</div><div className="sl" data-i18n="stats.daysStreak">{t('stats.daysStreak')}</div></div>
        </div>
      </div>
      ...
      <div className="stats-section">
        <div className="stats-section-title">
          ...
          <span id="chart-period-label">{t('stats.perDayCount', { n: chartDays, unit: pluralLabel('common_day', chartDays) })}</span>
        </div>
        <div className="chart-period-btns" id="chart-period-btns">
          {[14, 30, 90].map(d => (
            <button key={d} className={'chart-period-btn' + (chartDays === d ? ' active' : '')} data-days={d} onClick={() => setChartDays(d)}>{d}</button>
          ))}
        </div>
            ...
            {!hasChartData ? (
              ...
              return (
                <div className={'chart-col' + (sm ? ' chart-col-sm' : '')} key={d.date}>
                  {d.val > 0 ? <div className="chart-val">{d.val}</div> : <div className="chart-val" style={{ visibility: 'hidden' }}>0</div>}
                  <div className="chart-bar-wrap"><div className={'chart-bar' + (d.isToday ? ' today' : '')} style={{ height: h }} /></div>
                  <div className="chart-label">{d.isToday && !sm ? t('stats.today') : showLabel ? d.label : ''}</div>
                </div>
              );
            })}
          </div>
      ...
      <div className="stats-section">
        <div className="stats-section-title" data-i18n="stats.yearActivity">{t('stats.yearActivity')}</div>
        <div style={{ fontSize: '.68rem', color: 'var(--text3)', marginBottom: 6 }} data-i18n="stats.yearActivityDesc">{t('stats.yearActivityDesc')}</div>
          ...
          <div id="heatmap" style={{ display: 'flex', gap: 2, minWidth: 'max-content' }}>
            {heatmap.map((wk, wi) => (
              <div className="hm-week" key={wi}>
                {wk.map((day, di) => (
                  <div key={di} className={`hm-day hm-l${day.lvl}`} title={`${day.ds}: ${day.n} ${wordsLabel(day.n)}`} />
                ))}
              </div>
            ))}
          </div>
      ...
      <div className="stats-section">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div className="stats-section-title" style={{ marginBottom: 0 }} data-i18n="stats.monthlyView">{t('stats.monthlyView')}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <button id="cal-prev" className="btn" style={{ padding: '3px 10px', fontSize: 13 }} onClick={() => changeMonth(-1)}>←</button>
            <span id="cal-month-label" style={{ fontSize: '.82rem', fontWeight: 600, color: 'var(--text)', minWidth: 110, textAlign: 'center' }}>{monthNames()[calMonth]} {calYear}</span>
            <button id="cal-next" className="btn" style={{ padding: '3px 10px', fontSize: 13 }} onClick={() => changeMonth(1)}>→</button>
          </div>
        </div>
        <div className="cal-header-grid" id="cal-headers">
          {dowNames().map((d, i) => <div className="cal-dow" key={i}>{d}</div>)}
        </div>
        <div className="cal-day-grid" id="cal-grid">
          {calData.cells.map((c, i) => c === null
            ? <div className="cal-day cal-empty" key={i} />
            : (
              <div className={`cal-day hm-l${c.lvl}${c.isToday ? ' cal-today' : ''}`} title={`${c.ds}: ${c.n} ${wordsLabel(c.n)}`} key={i}>
                <span className="cal-day-num">{c.d}</span>
                {c.n > 0 && <span className="cal-day-cnt">{c.n}</span>}
              </div>
            ))}
        </div>
        <div id="cal-summary" style={{ fontSize: '.75rem', color: 'var(--text2)', marginTop: 8, textAlign: 'right' }}>{calData.summary}</div>
      </div>

      <div className="stats-section">
        <div className="stats-section-title" data-i18n="stats.bestTimeTitle">{t('stats.bestTimeTitle')}</div>
        <div id="hourly-chart" style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 48, marginBottom: 4 }}>
          {hourly.bars.map(b => (
            <div key={b.h} style={{ flex: 1, background: b.color, height: `${b.pct}%`, borderRadius: '2px 2px 0 0', opacity: b.n ? 1 : 0.2 }} title={`${b.h}:00 — ${b.n} ${wordsLabel(b.n)}`} />
          ))}
        </div>
        <div id="best-hour" style={{ fontSize: '.75rem', color: 'var(--text2)' }}>{hourly.bestLabel}</div>
      </div>

      <div className="stats-section">
        <div className="stats-section-title" data-i18n="stats.blockProgress">{t('stats.blockProgress')}</div>
        <div className="blocks-list" id="blocks-list">
          {blocks.map(b => (
            <div className="block-row" key={b.label}>
              <div className="block-label">{b.label}</div>
              <div className="block-track"><div className="block-fill" style={{ width: `${b.pct}%`, background: b.color }} /></div>
              <div className="block-pct" style={{ color: b.color }}>{b.pct}%</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="stats-section">
        <div className="stats-section-title" data-i18n="stats.srsForecastTitle">{t('stats.srsForecastTitle')}</div>
          ...
          <div className="srs-fc-bars">
            {srsFc.bars.map(c => (
              <div className="srs-fc-col" key={c.date}>
                <div className="srs-fc-bar-wrap"><div className={'srs-fc-bar' + (c.isToday ? ' srs-fc-today' : '')} style={{ height: `${c.pct}%` }} /></div>
      ...
      <div className="stats-section">
        <div className="stats-section-title" data-i18n="stats.modeAccuracyTitle">{t('stats.modeAccuracyTitle')}</div>
        <div id="mode-accuracy-list">
          {modeRows.length === 0 ? (
            <div style={{ fontSize: '.8rem', color: 'var(--text3)', textAlign: 'center', padding: '8px 0' }}>{t('stats.noModeData')}</div>
          ) : modeRows.map(m => (
            <div style={{ marginBottom: 10 }} key={m.key}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                <span style={{ fontSize: '.82rem', fontWeight: 600, color: 'var(--text)' }}>{m.icon} {m.label}</span>
                <span style={{ fontSize: '.75rem', color: 'var(--text2)' }}>{m.totText}{m.sessions ? ` · ${m.sessions} ${t('stats.sessionsAbbr')}` : ''}</span>
                <span style={{ fontSize: '.82rem', fontWeight: 700, color: m.barColor, minWidth: 36, textAlign: 'right' }}>{m.pct !== null ? `${m.pct}%` : '—'}</span>
              </div>
              <div style={{ height: 6, background: 'var(--border)', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${m.pct ?? 0}%`, background: m.barColor, borderRadius: 3, transition: 'width .4s' }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="stats-section">
        <div className="stats-section-title" data-i18n="stats.cefrProgressTitle">{t('stats.cefrProgressTitle')}</div>
        <div id="cefr-stats-list">
          {cefrRows.map(r => (
            <div style={{ marginBottom: 10 }} key={r.level}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                <span style={{ fontSize: '.8rem', fontWeight: 700 }}>
                  <span style={{ background: `${r.color}22`, color: r.color, border: `1.5px solid ${r.color}44`, borderRadius: 6, padding: '1px 6px', fontSize: '.72rem', marginRight: 6 }}>{r.level}</span>
                  {r.desc}
                </span>
                <span style={{ fontSize: '.75rem', color: 'var(--text2)' }}>{r.known} / {r.total} ({r.pct}%)</span>
              </div>
              <div style={{ height: 5, background: 'var(--border)', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${r.pct}%`, background: r.color, borderRadius: 3, transition: 'width .5s' }} />
              </div>
            </div>
          ))}
        </div>
