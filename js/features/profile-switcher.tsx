...
function _getProfiles(): Profile[] {
  try {
    return JSON.parse(localStorage.getItem(LIST_KEY) ?? '[]') as Profile[];
  } catch (e) {
    return [];
  }
}
function _setProfiles(p: Profile[]): void {
  localStorage.setItem(LIST_KEY, JSON.stringify(p));
}
function _getActiveId(): string {
  return localStorage.getItem(ACTIVE_KEY) ?? '';
}
function _setActiveId(id: string): void {
...
function _clearActiveKeys(): void {
  _snapKeys().forEach((k) => localStorage.removeItem(k));
}
...
function _ensureInit(): void {
  ...
  if (!profiles.length) {
    const id = 'p' + Date.now();
    profiles = [
      { id, name: t('duel.player'), avatar: '🧑', appearance: { ...DEFAULT_APPEARANCE } },
    ];
    ...
    if (!profiles.some((p) => p.id === aid)) {
      _setActiveId(profiles[0].id);
      _loadSnapshot(profiles[0].id);
    }
  }
}
...
export function ProfileSwitcher(): ReactElement {
  ...
  function confirmAdd(): void {
    ...
    if (!name) {
      ...
      setTimeout(() => setNewNameError(false), 1200);
      return;
    }
    _saveSnapshot(activeId);
    const id = 'p' + Date.now();
    ...
    const next = [
      ...profiles,
      { id, name, avatar: newAvatar, avatarMode, appearance: { ...DEFAULT_APPEARANCE } },
    ];
    _setProfiles(next);
    _setActiveId(id);
    _clearActiveKeys();
  ...
  function saveEdit(): void {
    ...
    if (!name) {
      ...
      setTimeout(() => setEditNameError(false), 1200);
      return;
    }
    ...
    const next = profiles.map((p) =>
      p.id === editTarget.id
        ? {
            ...
            name,
            avatar: editAvatar,
            ...(editAvatarTouched ? { avatarMode: 'preset' as const } : {}),
          }
        : p,
    );
    setProfiles(next);
        ...
        <div className="prf-char-preview">
          <ProfileAvatarView
            profile={{
              ...
              name: '',
              avatar: newAvatar,
              avatarMode: newAvatarTouched ? 'preset' : 'character',
              ...
              <div className="prf-char-preview" style={{ marginBottom: 14 }}>
                <ProfileAvatarView
                  profile={{
                    ...editTarget,
                    avatar: editAvatar,
                    avatarMode: editAvatarTouched ? 'preset' : editTarget.avatarMode,
                  }}
                  size={56}
                />
                <span className="prf-char-hint">{t('profile.customizeHint')}</span>
