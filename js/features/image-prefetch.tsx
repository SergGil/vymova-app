...
export function ImagePrefetchSettings(): ReactElement {
  ...
  function cachedCount(): number {
    return Object.keys(_imgCache).length;
  }
  function withImageCount(): number {
    return Object.keys(_imgCache).filter((k) => _imgCache[k]).length;
  }

  function findNext(): string | null {
    while (
      posRef.current < W.length &&
      Object.prototype.hasOwnProperty.call(
        _imgCache,
        (W[posRef.current] as unknown as WordEntry)[0],
      )
    )
      posRef.current++;
    ...
    <>
      <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginTop: '8px' }}>
        <input
          ...
          type="text"
          placeholder={t('settings.pixabayPlaceholder')}
          value={pixValue}
      ...
      <div style={{ fontSize: '.73rem', marginTop: '5px', color: pixStatusColor }}>
        {pixStatusText}
      </div>
