  ...
  if (cards.length === 0) {
    return (
      <Dialog open onOpenChange={(nextOpen) => !nextOpen && onClose()}>
        <DialogPortal>
          <DialogOverlay className="mistake-review-overlay bg-black/55 p-4" />
          <DialogPopup className="mistake-review-panel relative w-full max-w-[400px] rounded-[18px] bg-card px-5 pt-6 pb-5 shadow-[var(--mistake-panel-shadow)]">
            <button
              className="mistake-review-close absolute top-3 right-[14px] cursor-pointer border-none bg-transparent text-[1.1rem] leading-none text-text2"
              onClick={onClose}
  ...
  return (
    <Dialog
      ...
      onOpenChange={(nextOpen, eventDetails) => {
        if (nextOpen) return;
        // Same "no passive dismiss mid-review" rule the original overlay's
        // conditional onClick={!done ? undefined : onClose} enforced — only
        // the explicit ✕/OK buttons can close this while cards remain,
        // matching reset-confirm-dialog.tsx's escape-key cancel pattern
        // (base-ui has no declarative prop for "block outside-press only
        // sometimes", so both reasons are caught here directly).
        if (
          !done &&
    ...
    >
      <DialogPortal>
        <DialogOverlay className="mistake-review-overlay bg-black/55 p-4" />
        <DialogPopup className="mistake-review-panel relative w-full max-w-[400px] rounded-[18px] bg-card px-5 pt-6 pb-5 shadow-[0_8px_32px_rgba(0,0,0,0.25)]">
          <button
            className="mistake-review-close absolute top-3 right-[14px] cursor-pointer border-none bg-transparent text-[1.1rem] leading-none text-text2"
            onClick={onClose}
            aria-label={t('common.close')}
          ...
          {done ? (
            <div className="mistake-review-done pt-2.5 pb-1 text-center">
              <div className="mistake-review-done-title mb-1.5 text-[1.4rem] font-extrabold text-text">
            ...
            <>
              ...
              >
                ...
                <div
                  className={'mistake-review-back ' + FACE_BASE + (flipped ? ' block' : ' hidden')}
                >
                  ...
                  {card!.entry[2] && (
                    <div className="mistake-review-ex-row flex items-start justify-center gap-1.5">
                      ...
                      <button
                        type="button"
                        className="mistake-review-speak mistake-review-speak-ex mt-px mb-0 shrink-0 cursor-pointer border-none bg-transparent text-base"
