...
export function Leaderboard({ refreshKey }: { refreshKey: number }): ReactElement {
  ...
  if (state === 'loading') {
    return <div style={{ textAlign: 'center', padding: 20, color: 'var(--text3)' }}>{t('lb.loading')}</div>;
  }
  if (state === 'empty') {
    return <div style={{ textAlign: 'center', padding: 20, color: 'var(--text3)' }}>{t('lb.empty')}</div>;
  }
  ...
  return (
    <>
      <div style={{ marginBottom: 12, fontSize: '.78rem', color: 'var(--text3)', textAlign: 'center' }}>
        {t('lb.top20')}
      ...
      {entries.map((e, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 10, background: isMe ? 'rgba(0,200,100,.08)' : 'var(--bg)', border: `1.5px solid ${isMe ? 'var(--accent)' : 'var(--border)'}`, marginBottom: 6 }}>
            <span style={{ fontSize: '1rem', flexShrink: 0, minWidth: 28, textAlign: 'center' }}>{medal}</span>
            <span style={{ fontSize: '1.3rem' }}>{e.avatar}</span>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: isMe ? 700 : 400, fontSize: '.88rem', color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.name}{isMe ? ` (${t('lb.you')})` : ''}</div>
              <div style={{ fontSize: '.72rem', color: 'var(--text3)' }}>🔥 {e.streak} · 📚 {e.known} {wordsLabel(e.known)}</div>
            </div>
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              <div style={{ fontWeight: 700, fontSize: '.9rem', color: 'var(--accent)' }}>{e.xp} XP</div>
            </div>
          </div>
        );
      })}
