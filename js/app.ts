...
Object.keys(_loadedSrs).forEach(function(k: string){ if(typeof (_loadedSrs as any)[k]==='number') delete (_loadedSrs as any)[k]; });
...
window.addEventListener('ew-learn-lang-changed', function() {
  ...
  setTimeout(function() {
    try { updateSrsUI((getBaseWordsSnapshot() ?? W) as unknown as WordEntry[]); } catch (e) {}
  }, 0);
});
