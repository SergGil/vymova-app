...
export function ScramblePage(): ReactElement {
  ...
  const setupQuestion = (d: WordEntry[], i: number): void => {
    ...
    setAnswered(false); setHintsLeft(HINTS); setFailedThis(false); setResult(null);
  };

  const startGame = (): void => {
    const d = build();
    setDeck(d); setIdx(0); setOk(0); setFail(0); setCompleted(false);
    setupQuestion(d, 0);
  };
  ...
  const check = (lett: Tile[], ans: number[]): void => {
    ...
    if (a === target) {
      ...
      setTimeout(() => {
        setResult(null);
        setLetters(ls => { const reset = ls.map(t2 => ({ ...t2, used: false })); return reset; });
        setAnswer([]);
      }, 700);
    }
  };
  ...
  const deselectAt = (pos: number): void => {
    ...
    const newAnswer = answer.slice(); newAnswer.splice(pos, 1);
    setLetters(newLetters);
    setAnswer(newAnswer);
  };
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--text)' }}>🔀 <span data-i18n="mode.scramble">{t('mode.scramble')}</span></div>
          <div style={{ fontSize: '.75rem', color: 'var(--text3)', marginTop: 2 }}>
            {!showFinal ? `${t('scramble.word')} ${idx + 1} ${t('common.of')} ${SIZE}` : t('write.completed')}
          </div>
        </div>
        <button onClick={closeScramble} style={{ background: 'none', border: 'none', fontSize: '1.3rem', cursor: 'pointer', color: 'var(--text3)' }}>✕</button>
      </div>

      <div style={{ height: 4, background: 'var(--border)', borderRadius: 4, marginBottom: 12, overflow: 'hidden' }}>
        <div style={{ height: '100%', background: 'var(--accent)', borderRadius: 4, width: showFinal ? '100%' : `${idx / SIZE * 100}%`, transition: 'width .4s' }} />
      </div>
      ...
      {!showFinal && w && (
        <>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center', minHeight: 48, marginBottom: 14, borderBottom: '2px dashed var(--border)', paddingBottom: 14 }}>
            {answer.map((li, pos) => (
              <button key={'a' + pos} className="scr-tile scr-tile-placed" disabled={answered} onClick={() => deselectAt(pos)}>{letters[li].ch.toUpperCase()}</button>
            ))}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center', marginBottom: 8 }}>
            {tileOrder.filter(li => !letters[li].used).map(li => (
              <button key={'t' + li} className="scr-tile" disabled={answered} onClick={() => selectLetter(li)}>{letters[li].ch.toUpperCase()}</button>
            ))}
          </div>

          <div style={{ textAlign: 'center', fontSize: '.9rem', fontWeight: 600, minHeight: 24, marginBottom: 8 }}>
            {result && <span style={{ color: result.ok ? '#27ae60' : '#e74c3c' }}>{result.text}</span>}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            >{hintsLeft > 0 ? t('scramble.hintBtn', { n: hintsLeft }) : t('scramble.hintNone')}</button>
            <button
              onClick={shuffleTiles}
              disabled={answered}
              style={{ padding: '9px 14px', borderRadius: 10, border: '1.5px solid var(--border)', background: 'none', color: 'var(--text2)', cursor: 'pointer', fontFamily: 'inherit', fontSize: '.8rem' }}
              data-i18n="scramble.shuffleBtn"
            >{t('scramble.shuffleBtn')}</button>
            <button
