...
export function WordLettersPage(): ReactElement {
  ...
  const useHint = (): void => {
    ...
    const remaining = r.possible
      .filter((w) => !found.has(w) && !hintsShown.has(w))
      ...
      >
        <div>
          <div style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--text)' }}>
            🔤 <span data-i18n="mode.letters">{t('mode.letters')}</span>
          </div>
          <div style={{ fontSize: '.75rem', color: 'var(--text3)', marginTop: 2 }}>
            {!showFinal
              ? `${t('letters.round')} ${idx + 1} ${t('common.of')} ${ROUNDS}`
              : t('write.completed')}
          </div>
        </div>
        <button
      ...
      >
        <div
          style={{
            ...
            borderRadius: 4,
            width: showFinal ? '100%' : `${(idx / ROUNDS) * 100}%`,
            transition: 'width .4s',
          }}
        />
      </div>
      ...
      {!showFinal && r && (
        <>
          ...
          >
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center' }}>
              {tileOrder
                ...
                .map((ti) => (
                  <button key={'t' + ti} className="scr-tile min-w-[38px] h-[42px] cursor-pointer rounded-md border-2 border-[var(--border)] bg-[var(--card)] px-1 font-[inherit] text-[1.1rem] font-bold text-[var(--text)] transition-[transform,background,border-color] duration-150 hover:-translate-y-0.5 hover:border-[var(--accent)] disabled:cursor-default disabled:opacity-85" onClick={() => select(ti)}>
          ...
          >
            ...
            {!guess.length && <span className="scr-tile scr-tile-empty min-w-[38px] h-[42px] rounded-md border-2 border-dashed border-[var(--border)] bg-transparent" />}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            >
              {hintsLeft > 0 ? t('letters.hintBtn', { n: hintsLeft }) : t('letters.hintNone')}
            </button>
            <button
            ...
            >
              {idx >= ROUNDS - 1 ? t('quiz.finish') : t('letters.doneBtn')}
            </button>
          </div>
        </>
      )}
      ...
      {showFinal && (
        <div style={{ textAlign: 'center', padding: '10px 0 4px' }}>
          ...
          <div style={{ fontSize: '.9rem', color: 'var(--text2)', marginBottom: 16 }}>
            {foundTotal} {t('common.of')} {possibleTotal} ({pct}%)
          </div>
