...
export function ReadingPage(): ReactElement {
  ...
  const handleEpubChange = (e: { target: HTMLInputElement }): void => {
    ...
    loadEpub(file,
      (msg: string, pct: number) => { setEpubProgress(`${msg} (${pct}%)`); },
      (chunks: string[] | null, err: string | null) => {
        ...
        setEpubBook({ title: bookTitle, chapters: chunks.map(text => ({ text, title: bookTitle, level: 'epub' })) });
        setCurrentTextIdx(0);
        setEpubProgress(t('reading.epubLoaded', { n: chunks.length, unit: pluralLabel('common_fragment', chunks.length) }));
  ...
  const title = epubBook ? `${epubBook.title} — ${t('reading.chapterLabel', { n: currentTextIdx + 1 })}` : entry.title;
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <div className="page-title" data-i18n="reading.title">{t('reading.title')}</div>
          <div style={{ fontSize: '.72rem', color: 'var(--text3)', marginTop: 2 }}>{t('reading.statsLine', { k: known, u: unknown })}</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button
            className="backup-btn" style={{ padding: '5px 12px' }}
            disabled={currentTextIdx === 0}
            onClick={() => setCurrentTextIdx(i => i - 1)}
            data-i18n="reading.prevBtn"
          >{t('reading.prevBtn')}</button>
          <span style={{ fontSize: '.7rem', padding: '2px 8px', borderRadius: 20, background: 'var(--accent)', color: '#fff', fontWeight: 700 }}>{level}</span>
          <button
            className="backup-btn" style={{ padding: '5px 12px' }}
            disabled={currentTextIdx === texts.length - 1}
            onClick={() => setCurrentTextIdx(i => i + 1)}
            data-i18n="reading.nextBtn"
          >{t('reading.nextBtn')}</button>
          <button className="page-close-btn" onClick={closeReading}>✕</button>
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        <h2 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text)', flex: 1, minWidth: 0, margin: 0 }}>{title}</h2>
        <button className="backup-btn primary" style={{ padding: '6px 14px', fontSize: '.78rem', flexShrink: 0 }} onClick={() => document.getElementById('rd-epub-input')?.click()} data-i18n="reading.epubBtn">{t('reading.epubBtn')}</button>
        <input id="rd-epub-input" type="file" accept=".epub" style={{ display: 'none' }} onChange={handleEpubChange} />
      </div>
      {epubProgress && (
        <div style={{ fontSize: '.75rem', color: 'var(--accent)', marginBottom: 8, padding: '6px 10px', background: 'rgba(0,200,255,.07)', borderRadius: 8 }}>{epubProgress}</div>
      )}
