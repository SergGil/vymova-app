...
export function WritePage(): ReactElement {
  ...
  const resetQ = (): void => {
    setInput(''); setBorderColor('var(--border)'); setResult(null); setHint('');
    setAnswered(false); acHide();
  };

  const startGame = (src?: WordEntry[] | null): void => {
    setDeck(build(src));
    setIdx(0); setOk(0); setFail(0); setWrong([]); setCompleted(false);
    resetQ();
  };
  ...
  const submit = (): void => {
    ...
    setTimeout(() => { try { nextRef.current?.focus(); } catch (e) {} }, 0);
  };
  ...
  const pickAc = (i: number): void => {
    if (acItems[i]) { setInput(acItems[i][0]); acHide(); try { inputRef.current?.focus(); } catch (e) {} }
  };
  ...
  const onInputKeydown = (e: KeyboardEvent | { key: string; preventDefault: () => void }): void => {
    ...
    if (acItems.length && (ev.key === 'ArrowDown' || ev.key === 'ArrowUp')) {
      ev.preventDefault();
      const next = ev.key === 'ArrowDown' ? Math.min(acIdx + 1, acItems.length - 1) : Math.max(acIdx - 1, -1);
      setAcIdx(next);
      if (next >= 0) setInput(acItems[next][0]);
    } else if (ev.key === 'Escape') {
      acHide();
    } else if (ev.key === 'Enter') {
      if (acIdx >= 0 && acItems[acIdx]) { setInput(acItems[acIdx][0]); acHide(); }
      else if (!answered) { acHide(); submit(); }
      else { acHide(); advanceQ(); }
    }
  };
  ...
  const startMic = (): void => {
    ...
    recog.onend = () => {
      setMicActive(false);
      setInput(curr => { if (curr.trim()) setTimeout(submit, 0); return curr; });
    };
    recog.start();
  };
  ...
  useEffect(() => {
    function onKeydown(e: KeyboardEvent): void {
      ...
      if (e.key === 'Escape' && acItems.length) { acHide(); return; }
      if (e.key === 'Escape') { closeWrite(); return; }
      if ((e.key === 'ArrowRight' || e.key === ' ') && answered && document.activeElement !== inputRef.current) {
  ...
  const finalTitle = pct === 100 ? t('quiz.perfectTitle') : pct >= 80 ? t('quiz.greatTitle') : pct >= 60 ? t('quiz.goodTitle') : t('quiz.encourageTitle');

  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--text)' }} data-i18n="write.title">{t('write.title')}</div>
          <div style={{ fontSize: '.75rem', color: 'var(--text3)', marginTop: 2 }}>
            {!showFinal && w ? `${t('quiz.question')} ${idx + 1} ${t('common.of')} ${deck.length}` : showFinal ? t('write.completed') : ''}
          </div>
        </div>
        <button onClick={closeWrite} style={{ background: 'none', border: 'none', fontSize: '1.3rem', cursor: 'pointer', color: 'var(--text3)' }}>✕</button>
      </div>
        ...
        <>
          ...
          <div style={{ position: 'relative', marginBottom: 10 }}>
            <input
              ...
              style={{
                width: '100%', padding: '12px 16px', border: `2px solid ${borderColor}`, borderRadius: 12,
                fontSize: '1rem', fontFamily: "'DM Sans',sans-serif", background: 'var(--bg)', color: 'var(--text)',
            ...
            {acItems.length > 0 && (
              <div className="write-ac" style={{ display: 'block' }}>
                {acItems.map((aw, i) => (
                  <div key={aw[0]} className={'wac-item' + (i === acIdx ? ' wac-sel' : '')} onClick={() => pickAc(i)}>
                    <span className="wac-word">{aw[0]}</span><span className="wac-ua">{aw[1]}</span><span className="wac-n">{i + 1}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ fontSize: '.72rem', color: 'var(--text3)', marginBottom: 8, textAlign: 'center', minHeight: 16 }}>{hint}</div>
          <div style={{ minHeight: 28, textAlign: 'center', fontSize: '.9rem', fontWeight: 600, marginBottom: 12 }}>
            {result && <span style={{ color: result.color, fontSize: '1.05rem' }} dangerouslySetInnerHTML={{ __html: result.text }} />}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
            {!answered && (
              <button
                onClick={submit}
                style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '.9rem', fontWeight: 600, padding: '11px 32px', borderRadius: 10, border: 'none', background: 'var(--accent)', color: '#fff', cursor: 'pointer' }}
            ...
            {SpeechRec && (
              <button
                ...
                style={{
                  fontFamily: "'DM Sans',sans-serif", fontSize: '.9rem', padding: '11px 14px', borderRadius: 10,
                  border: `1.5px solid ${micActive ? '#e74c3c' : 'var(--border)'}`, background: 'none',
                  color: micActive ? '#e74c3c' : 'var(--text2)', cursor: 'pointer', transition: 'all .2s',
                }}
              >{micActive ? '🔴' : '🎤'}</button>
            )}
            <button
              onClick={showHint}
              style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '.82rem', padding: '11px 18px', borderRadius: 10, border: '1.5px solid var(--border)', background: 'none', color: 'var(--text3)', cursor: 'pointer' }}
            >💡</button>
          </div>
        </>
      )}
        ...
        <div style={{ textAlign: 'center', padding: '10px 0 4px' }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            {wrong.length > 0 && (
              ...
              >{t('write.mistakesBtn')} ({wrong.length})</button>
            )}
            <button
              onClick={closeWrite}
