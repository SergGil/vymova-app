...
33. [x] Спостерігач, асинхронні челенджі, турнірна сітка —
    **спостерігач мігровано**. `_renderSpectatorView(room)` тепер лише
    ...
    `refreshDuelSpectator()`; `duel-spectator.tsx` (`DuelSpectatorView`,
    `mountDuelSpectator()`/`refreshDuelSpectator()`) рендерить аватари/
    рахунок/прогрес-крапки/кнопку "Покинути" (`_leaveSpectator()`,
...
34. [x] Перенести shell у єдиний React-рут (`createRoot` на `#app-root`) —
    **виконано через портал-підхід**. Замість переписування `app.ts`/всіх
    `mountXxx()` одночасно (надто ризиковано), реалізовано
    `src/app-root.tsx`: один `createRoot()` на новому невидимому
    `<div id="app-root" style="display:none">`, який рендерить
    `<AppRoot/>` — плоский список `<Portal id="xxx-mount"><Component/></Portal>`
    (всього 50+ порталів). `Portal` — тонкий `createPortal(children,
    document.getElementById(id))`, тож вся існуюча розмітка/CSS/z-index не
    змінюються. Кожен з ~43 `.tsx`-файлів втратив власний `createRoot`/
    `mountXxx()`/`Root`-змінну і отримав `export` на компоненті(-ах);
    refresh-pattern-компоненти (duel-*, achievements, card-progress тощо)
    конвертовано з `_xxxRoot.render(...)` на `useStateVersion()` +
    `refreshXxx(): void { notifyStateChange(); }` через спільний
    `useSyncExternalStore`-механізм (`src/store.ts`). `src/main.ts` тепер
...
---
**Items 35/36 — майбутній окремий план**: ці два пункти тісно пов'язані —
залишок `$e`/`querySelector`/`addEventListener` (35) майже завжди є
зворотною стороною `window.*`-інтерфейсу (36): оверлей відкривається/
закривається через DOM-listener, а стан і рефреш йдуть через `window.*`
(`app.ts`, `duel.ts`, `sidebar.ts`, `daily-challenge.ts`, card-actions,
swipe, keyboard). Повне закриття обох пунктів вимагає одного спільного
проєкту переписування цих ядрових imperative-модулів на React (з власним
сховищем стану замість `window.*`), а не двох окремих задач — інакше
проміжний стан буде непослідовним. Коли буде ресурс на таку роботу, варто
оформлено окремим документом — `IMPERATIVE_CORE_MIGRATION_PLAN.md`,
з власною Фазою 7 (підпункти 7.1-7.5), що охоплює items 35+36 разом.
...
частиною картки (items 24-28) — усе покрите тестами (529/529). Item 34
(єдиний React-рут) виконано портал-підходом (`src/app-root.tsx`,
`mountAppRoot()`) — усі ~43 React-компоненти тепер монтуються одним
`createRoot()` через `createPortal` у свої існуючі `#xxx-mount` точки.
Лише короткі async-challenge флоу (`createAsyncChallenge`/
`joinAsyncChallenge`) і items 35-36 свідомо залишені imperative — перші
не мають власної розмітки, що варто було б мігрувати, а 35-36 — це
